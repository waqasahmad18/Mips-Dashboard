...
export function SinglePerformanceChart({ onTabChange, onCategoryChange, onQuarterChange }: SinglePerformanceChartProps) {
  ...
  const [selectedQuarter, setSelectedQuarter] = useState<QuarterTab>("Q1");
  ...
  const comparisonQuarterStore = useMemo(() => {
    if (selectedCategory === "quality") {
      return {
        "2024": { Q1: 93, Q2: 91, Q3: 96, Q4: 95, overall: 95 },
        "2025": { Q1: 94, Q2: 93, Q3: 97, Q4: 96, overall: 96 },
        "2026": { Q1: 95, Q2: null, Q3: null, Q4: null, overall: 95 },
      } as const;
    }
    ...
    if (selectedCategory === "cost") {
      return {
        ...
        "2025": { Q1: null, Q2: null, Q3: null, Q4: null, overall: null },
        "2026": { Q1: null, Q2: null, Q3: null, Q4: null, overall: null },
      } as const;
    }

    return {
      "2024": { Q1: 88, Q2: 90, Q3: 92, Q4: 93, overall: 91 },
      "2025": { Q1: 91, Q2: 92, Q3: 94, Q4: 95, overall: 93 },
      "2026": { Q1: 95, Q2: null, Q3: null, Q4: null, overall: 95 },
    } as const;
  }, [selectedCategory]);

  const comparisonQuarterData = useMemo(() => {
    const quarterOrder =
      selectedQuarter === "Overall"
        ? QUARTERS
        : QUARTERS.slice(0, QUARTERS.indexOf(selectedQuarter as Exclude<QuarterTab, "Overall">) + 1);

    const shouldHide2026 = selectedQuarter === "Q2" || selectedQuarter === "Q3" || selectedQuarter === "Q4";
    return quarterOrder.map((quarter) => ({
      quarter,
      y2024: comparisonQuarterStore["2024"][quarter],
      y2025: comparisonQuarterStore["2025"][quarter],
      y2026: shouldHide2026 ? null : comparisonQuarterStore["2026"][quarter],
    }));
  }, [comparisonQuarterStore, selectedQuarter]);

  const comparisonYearBars = useMemo(() => {
    if (selectedQuarter === "Overall") {
      return (["2024", "2025", "2026"] as const)
        .map((year) => ({
          year,
          value: comparisonQuarterStore[year].overall,
        }))
    ...
    return (["2024", "2025", "2026"] as const)
      .map((year) => ({
        year,
        value: comparisonQuarterStore[year][selectedQuarter],
      }))
      .filter((item) => item.value !== null);
  }, [comparisonQuarterStore, selectedQuarter]);
  ...
  const overallQuarterData = useMemo(
    () => [
      { quarter: "Q1", total: Number(overallScore.toFixed(2)) },
      { quarter: "Q2", total: null },
      { quarter: "Q3", total: null },
      { quarter: "Q4", total: null },
    ],
    [overallScore],
  );
  ...
  const isGroupedOnlyCategory = selectedCategory === "pi" || selectedCategory === "ia";

  return (
    <section className="relative mt-8 overflow-hidden rounded-2xl border border-sky-100 bg-white/85 px-5 pb-5 pt-7 shadow-2xl shadow-sky-100/60 backdrop-blur-sm dark:border-slate-700 dark:bg-slate-900/70">
      ...
      <div className="pb-4">
            <div className="flex flex-nowrap items-center justify-between gap-2">
              ...
              >
                ...
                <p className="mt-2 text-base font-bold">15%of 15%</p>
                <p className="mt-1 text-xs text-white/90">Category weight</p>
              </button>
              <span className="px-0.5 text-2xl font-black text-slate-500">+</span>
              <button
      ...
      <div className="mt-6 flex justify-center">
        <div className="inline-flex rounded-xl border border-slate-200 bg-white/80 p-1 shadow-sm dark:border-slate-700 dark:bg-slate-800/70">
          {(["Q1", "Q2", "Q3", "Q4", "Overall"] as const).map((quarter) => (
            <button
              key={quarter}
              type="button"
      ...
      {selectedQuarter === "Overall" &&
        activeTab === "performance" &&
        (selectedCategory === "cost" ? (
          <div className="mt-6 h-[380px] w-full rounded-2xl border border-slate-300/80 bg-gradient-to-b from-white via-slate-50/60 to-slate-100/40 p-4 shadow-[0_18px_40px_rgba(100,116,139,0.15)] ring-1 ring-white/80">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={[]} margin={{ top: 16, right: 8, left: 0, bottom: 10 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#cbd5e1" />
                <XAxis dataKey="quarter" tick={{ fill: "#111827", fontSize: 12, fontWeight: 600 }} />
            ...
            <p className="mt-2 text-center text-sm font-semibold text-slate-600">
              Cost Overall structure ready (blank chart).
            </p>
          </div>
        ) : (
          <div className="mt-6 h-[380px] w-full rounded-2xl border border-slate-300/80 bg-gradient-to-b from-white via-slate-50/60 to-slate-100/40 p-4 shadow-[0_18px_40px_rgba(100,116,139,0.15)] ring-1 ring-white/80">
            <div className="mb-2 text-center">
              <p className="text-base font-bold text-slate-800">Overall Quarterly Total Result</p>
              <p className="text-xs font-semibold text-slate-500">Q1 total is available; Q2, Q3, and Q4 are currently blank.</p>
            </div>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={overallQuarterData} margin={{ top: 16, right: 8, left: 0, bottom: 10 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#cbd5e1" />
                <XAxis dataKey="quarter" tick={{ fill: "#111827", fontSize: 12, fontWeight: 600 }} />
                <YAxis domain={[0, 100]} tick={{ fill: "#111827", fontSize: 12, fontWeight: 600 }} />
                <Tooltip
      ...
      {selectedQuarter === "Q1" && activeTab === "performance" && isCategoryReady && !isGroupedOnlyCategory && (
      ...
      isGroupedOnlyCategory && activeTab === "performance" ? null : activeTab === "performance" ? (
        ...
        <div className="mt-4 h-[380px] w-full rounded-2xl border border-sky-200/80 bg-gradient-to-b from-white via-sky-50/60 to-sky-100/40 p-4 shadow-[0_18px_40px_rgba(14,165,233,0.18)] ring-1 ring-white/80">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={comparisonYearBars} margin={{ top: 14, right: 14, left: 6, bottom: 10 }} barCategoryGap="26%">
              <defs>
                <linearGradient id="yearBar2024" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor="#f59e0b" />
                  <stop offset="100%" stopColor="#f97316" />
                </linearGradient>
                <linearGradient id="yearBar2025" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor="#22d3ee" />
                  <stop offset="100%" stopColor="#0891b2" />
                </linearGradient>
                <linearGradient id="yearBar2026" x1="0" y1="0" x2="0" y2="1">
              ...
              <YAxis domain={[0, 100]} tick={{ fill: "#111827", fontSize: 12, fontWeight: 600 }} />
              <Tooltip
                contentStyle={{ backgroundColor: "#ffffff", border: "1px solid #cbd5e1", borderRadius: 12, color: "#0f172a" }}
                formatter={(value) => {
                  const parsed = typeof value === "number" ? value : Number(value);
                  if (!Number.isFinite(parsed)) return "";
                  return `${Number.isInteger(parsed) ? parsed : parsed.toFixed(2)}%`;
      ...
      ) : selectedQuarter === "Overall" ? null : (
        <div className="mt-4 rounded-2xl border border-slate-200 bg-white/90 p-8 text-center shadow-sm">
          <p className="text-lg font-bold text-slate-800">
            {selectedQuarter} data view is coming next.
          </p>
          <p className="mt-2 text-sm text-slate-500">
            Q1 remains the active quarter for current chart rendering.
          </p>
        </div>
      )}
    </section>
